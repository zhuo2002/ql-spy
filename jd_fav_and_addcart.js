...
if ($.isNode()) {
  ...
  if (process.env.fav_and_add_cart_activityId)  $.activityId = process.env.fav_and_add_cart_activityId
  if (process.env.fav_and_add_cart_shareUuid)  $.shareUuid = process.env.fav_and_add_cart_shareUuid
  if (process.env.fav_and_add_cart_venderIds)  $.venderIds = process.env.fav_and_add_cart_venderIds
...
!(async() => {
    ...
    if (!$.activityId) {
        $.log(`\n甘露殿【https://t.me/jdredrain】提醒你:暂时没有关注+加购+分享领豆任务，改日再来～`);
        return;
    }
    console.log(`\n甘露殿【https://t.me/jdredrain】提醒你:新的关注+加购+分享领豆已经准备好: ${$.activityId}，准备开始薅豆`);
    ...
    if (cookie) {
        ...
        $.hotFlag = false;
        $.nickName = '';
        console.log(`\n\n******开始领取【京东账号${$.index}】${$.nickName || $.UserName}*********\n`);
		allMessage+=`\n******开始领取【京东账号${$.index}】${$.nickName || $.UserName}******\n`;
        await getUA()
        await run();
    }
      ...
      case 'getDrawRecordHasCoupon':
        if(typeof res == 'object'){
          if(res.result && res.result === true){
            ...
            if(value > 0){ 
				console.log(`共获得${parseInt(value, 10) || 0}京豆`);
				allMessage+=`共获得${parseInt(value, 10) || 0}京豆`+"\n";
			}
          }else if(res.errorMessage){
            console.log(`${type} ${res.errorMessage || ''}`)
          }else{
